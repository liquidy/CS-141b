...
implements CollaboratorService {
		...
		while (true) {
			...
			try {
				...
				if (numPeopleInFront == 0) {
					...
					messages.put(clientId, Messages.CODE_LOCK_READY + docKey);
				}
	...
	public String unrequestDocument(String docKey, String token) {
		...
		while (true) {
			...
			try {
				...
				while (clientsItr.hasNext()) {
					String clientId = clientsItr.next();
					messages.put(clientId, Messages.CODE_LOCK_NOT_READY + 
	...
	protected static void pollDocQueue(Document doc, boolean lockExpired, PersistenceManager pm,
		...
		if (clientId != null) {
			if (lockExpired) {
				messages.put(clientId, Messages.CODE_LOCK_EXPIRED + docKey);
			}
	...
	private void removeClientFromTimeoutQueue(String docKey, PersistenceManager pm) {
		if (docKey != null) {
